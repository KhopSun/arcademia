...
import { Lesson, lessonsData } from "@/app/lib/lessons";
...
const LessonNode: React.FC<LessonNodeProps> = ({ lesson, isFirst, isLast }) => {
  ...
  const getIcon = () => {
    if (lesson.status === "locked")
      return <i className="nes-icon close is-medium w-5 h-5" />;
    if (lesson.status === "completed")
      return <i className="nes-icon star is-medium w-5 h-5" />;
    if (lesson.status === "current")
      return <i className="w-5 h-5 nes-icon star is-medium is-transparent" />;
    ...
    <div className="flex flex-col items-center w-full max-w-xs sm:max-w-sm md:max-w-md">
      ...
      <div className="flex flex-col items-center relative">
        <button
          ...
          className={`group relative w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center border-4 shadow-md focus:outline-none focus:ring-2 focus:ring-offset-2 ${statusStyles} ${
        ...
        <div
          className={`mt-2 px-3 py-1 rounded border-2 font-bold text-sm shadow-md 
          ${
            lesson.status === "locked"
              ? "bg-gray-300 border-gray-400 text-gray-500"
              : "bg-[#e5d9c4] border-black text-gray-800"
          }`}
        >
          {lesson.title}
        </div>
